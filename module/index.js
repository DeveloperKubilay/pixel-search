...
function takescreenshot(x) {
    const screenshotPath = path.join(process.cwd(), x);

    exec(`powershell -command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('%{PRTSC}'); Start-Sleep -Milliseconds 500; [System.Windows.Forms.Clipboard]::GetImage().Save('${screenshotPath}')"`, (error) => {
        if (error) { }
    });
}

module.exports = { findImageInImage, takescreenshot };